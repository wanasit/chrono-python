...
def has_implied_earlier_reference_date(result: ParsedResult) -> bool:
    return re.search(r"\s+(prima|da)$", result.text, re.IGNORECASE) is not None


def has_implied_later_reference_date(result: ParsedResult) -> bool:
    return re.search(r"\s+(dopo|dal?)$", result.text, re.IGNORECASE) is not None


class ITMergeRelativeFollowByDateRefiner(AbstractMergingRefiner):
    """
