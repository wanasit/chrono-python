...
def compile_pattern(allow_abbreviations: bool = True) -> re.Pattern:
    time_units_pat = constants.TIME_UNITS_PATTERN if allow_abbreviations else constants.TIME_UNITS_NO_ABBR_PATTERN
    return re.compile(
        r"(?:il\s*|la\s*|l'\s*)?(questo|questa|quest'|scorso|scorsa|prossimo|prossima|dopo|\+|-)\s*"
        rf"({time_units_pat})(?=\W|$)",
        re.IGNORECASE,
    )


class ITTimeUnitCasualRelativeParser(AbstractParserWithWordBoundary):
