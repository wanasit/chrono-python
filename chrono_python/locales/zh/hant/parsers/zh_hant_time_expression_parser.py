...
from chrono_python.locales.zh.hant.constants import NUMBER, zh_string_to_number


class ZHHantTimeExpressionParser(chrono.Parser):
    def __init__(self):
        num_chars = "".join(NUMBER.keys())
        date_pattern = (
            rf"(?:"
            rf"(今|明|聽|前|大前|後|大後|昨|尋|琴)(早|朝|晚)|"
            rf"(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|"
            rf"(今|明|聽|前|大前|後|大後|昨|尋|琴)(?:日|天)"
            rf"(?:[\s,，]*)"
            rf"(?:(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?"
            rf")?"
        )
        time_pattern = (
            rf"(?:(\d+|[{num_chars}]+)(?:\s*)(?:點|時|:|：)"
            rf"(?:\s*)"
        ...
        self._first_pattern = re.compile(
            rf"(?:從|自)?{date_pattern}(?:[\s,，]*){time_pattern}",
            re.IGNORECASE,
        )
    ...
    def extract(self, context: chrono.ParsingContext, match: chrono.Match) -> chrono.ParsedResult | None:
        ...
        if ampm_str:
            ...
            if zh_char in ("上", "早", "朝", "凌"):
                meridiem = 0
                if hour == 12:
                    hour = 0
            elif zh_char in ("下", "晏", "晚"):
                meridiem = 1
                if hour != 12:
                    hour += 12
        elif zh_ampm_g3:
        ...
        if end_day_g1:
            if end_day_g1 in ("明", "聽"):
                if ref_dt.hour > 1:
                    end_dt += datetime.timedelta(days=1)
            elif end_day_g1 in ("昨", "尋", "琴"):
                end_dt -= datetime.timedelta(days=1)
            elif end_day_g1 == "前":
            ...
            if end_day_g3 in ("明", "聽"):
                end_dt += datetime.timedelta(days=1)
            elif end_day_g3 in ("昨", "尋", "琴"):
                end_dt -= datetime.timedelta(days=1)
            elif end_day_g3 == "前":
                end_dt -= datetime.timedelta(days=2)
            elif end_day_g3 == "大前":
