...
class ZHHantDeadlineFormatParser(chrono.Parser):
    def __init__(self):
        num_chars = "".join(NUMBER.keys())
        self._pattern = re.compile(
            rf"(\d+|[{num_chars}]+|半|幾)(?:\s*)"
            rf"(?:個)?"
            rf"(秒(?:鐘)?|分鐘|小時|鐘|日|天|星期|禮拜|月|年)"
            rf"(?:(?:之|過)?後|(?:之)?內)",
            re.IGNORECASE,
        )
