...
class Chrono:
    ...
    def parse_date(self, text, reference: Moment | datetime.datetime | None = None) -> datetime.datetime | None:
        results = self.parse(text, reference)
        if len(results) > 0:
            return results[0].datetime()
        return None

    def parse(self, text, reference: Moment | datetime.datetime | None = None) -> list[ParsedResult]:
